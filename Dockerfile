...
RUN echo "cgi.fix_pathinfo=1" > ${php_vars} &&\
    echo "upload_max_filesize = 51200M"  >> ${php_vars} &&\
    echo "post_max_size = 51200M"  >> ${php_vars} &&\
    echo "variables_order = \"EGPCS\""  >> ${php_vars} && \
    echo "memory_limit = 512M"  >> ${php_vars} && \
    echo "max_execution_time = 36000"  >> ${php_vars} && \
    echo "max_input_time = 36000"  >> ${php_vars} && \
    sed -i \
...
COPY tinyfilemanager/ /usr/src/kodexplorer/
COPY entrypoint.sh /
RUN set -ex; \
    apk add --no-cache --virtual .fetch-deps \
    ; \
    ls -alF /usr/src/kodexplorer/ && \
